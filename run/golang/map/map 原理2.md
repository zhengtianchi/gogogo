...
>
>
>但是每个bmap桶中还有8个元素，并且还有可能有溢出桶，在每个桶中查找key时并不是将传入的key和桶中的key进行hash运算然后做比较因为做hash运算太耗时了。
>
>所以这里利用了tophash数组在map赋值时就已经提前计算好了key的hash值并将高8位存入tophash中，这样在比较时直接和tophash里的值进行比较，可以大大的节约时间，相当于用空间换时间。当然在进行查找key时tophash比较结果相同后还有再次对传入的key和桶中的key进行hash运算比较，避免出现高8位hash冲突的情况，虽然几率很小。
...
例如，现在有一个 key 经过哈希函数计算后，得到的哈希结果是： (第一步：计算key hash 值)
...
- 用最后的 `5 个 bit 位 （B）`，也就是 01010，值为 10，也就是 10 号桶。这个操作实际上就是取余操作，但是取余开销太大，所以代码实现上用的`位操作`代替。 (hmap 查找 bmap 位置) (第二步，取最后 B 位 位操作，确定在哪个 bmap (bucket) 中)
- 再用哈希值的`高 8 位`，找到此 key 在 bucket 中的位置，这是在寻找已有的 key。最开始桶内还没有 key，新加入的 key 会找到第一个空位，放入。 （bmap 查找 key） 
